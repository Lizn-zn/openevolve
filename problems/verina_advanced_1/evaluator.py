...
def evaluate(program_path: str) -> EvaluationResult:
    ...
    try:
        ...
        revised = False
        revised_nodes = []
        llm_revision_response = ""  # LLM 修复的原始 response
        
        # 如果第一遍编译失败，尝试 revise（用于打分和保存）
        if not first_is_valid_with_sorry and first_error_messages:
            revised_code, revise_success, revised_nodes, llm_revision_response = revise_proof(original_code, first_error_messages)
        ...
        # ===== 添加详细日志 artifacts（以 _ 开头，只保存不送入模型） =====
        eval_end_time = _time.time()
        
        # 评估耗时
        artifacts["_eval_time_seconds"] = f"{eval_end_time - eval_start_time:.2f}"
        
        # 原始编译结果
        artifacts["_original_compiler_result"] = str(first_result) if first_result else ""
        
        # 完整的错误消息列表（不只是前5个摘要）
        if first_error_messages:
            artifacts["_all_error_messages"] = "\n---\n".join(first_error_messages)
        
        # 所有 unsolved goals（不只是最难的那个）
        if first_unsolved_goals:
            all_goals_str = "\n---\n".join([
                f"[line {g.get('line', '?')}] {g.get('full_goal', g.get('goal', ''))}"
                for g in first_unsolved_goals
            ])
            artifacts["_all_unsolved_goals"] = all_goals_str
        
        # 如果有 revise，保存 revise 相关信息
        if revised:
            artifacts["_revised_code"] = scoring_code
            artifacts["_revised_compiler_result"] = str(scoring_result) if scoring_result else ""
            # revise 策略（llm 还是 sorry）
            if revised_nodes == "llm":
                artifacts["_revise_strategy"] = "llm"
            elif isinstance(revised_nodes, list):
                artifacts["_revise_strategy"] = f"sorry: {', '.join(revised_nodes)}"
            else:
                artifacts["_revise_strategy"] = str(revised_nodes)
            if llm_revision_response:
                artifacts["_llm_revision_response"] = llm_revision_response
            # revise 后的 unsolved goals
            if scoring_result:
                revised_unsolved_goals = extract_unsolved_goals(scoring_result)
                if revised_unsolved_goals:
                    revised_goals_str = "\n---\n".join([
                        f"[line {g.get('line', '?')}] {g.get('full_goal', g.get('goal', ''))}"
