...
/-- A general helper: `count a xs > 0` iff `a` occurs in `xs`. -/
lemma List.count_pos_iff_exists_mem {α : Type} [DecidableEq α]
    (xs : List α) (a : α) :
    0 < xs.count a ↔ ∃ x ∈ xs, x = a := by
  classical
  induction xs with
  | nil =>
      constructor
  ...
  | cons y ys ih =>
      ...
      · intro h
        ...
        · refine ⟨y, ?_, ?_⟩
          · simp
          · simpa [hya]
        · have : 0 < ys.count a := by
            -- Using `List.count_cons` and the assumption `y ≠ a`
            have := h
            simp [List.count, hya, Nat.succ_le_succ_iff] at this
            exact this
          rcases (ih.mp this) with ⟨x, hx_mem, hx_eq⟩
          exact ⟨x, by simp [hx_mem], hx_eq⟩
      · intro h
        rcases h with ⟨x, hx_mem, hx_eq⟩
        rcases hx_mem with rfl | hx_tail
        · subst hx_eq
          simp [List.count, Nat.zero_lt_succ]
        · have : 0 < ys.count a := (ih.mpr ⟨x, hx_tail, hx_eq⟩)
          by_cases hya : y = a
          · -- then `count a (y :: ys) = ys.count a + 1`
            have : 0 < ys.count a + 1 := Nat.succ_pos _
            simpa [List.count, hya, Nat.add_comm] using this
          · have : 0 < ys.count a := this
            -- then `count a (y :: ys) = ys.count a`
            simpa [List.count, hya] using this

/-- Specialized helper: if `xs.count a = 1`, then `a ∈ xs`. -/
lemma List.mem_of_count_eq_one {α : Type} [DecidableEq α]
    (xs : List α) (a : α) (h : xs.count a = 1) :
    a ∈ xs := by
  have : 0 < xs.count a := by
    simpa [h]
  rcases (List.count_pos_iff_exists_mem xs a).mp this with ⟨x, hx_mem, hx_eq⟩
  subst hx_eq
  exact hx_mem
...
lemma FindSingleNumber_precond_implies_exists_count1
  ...
  have h_mem_one : 1 ∈ nums.map (fun x => nums.count x) := by
    -- Use the generic "count = 1 implies membership" lemma on the mapped list.
    exact List.mem_of_count_eq_one (nums.map (fun x => nums.count x)) 1 h_unique
